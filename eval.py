...
parser = argparse.ArgumentParser(description='Focal-WNet training on KITTI/NYU',
...
def main():
    ...
    if args.img_save is True:
        ...
        for idx, (rgb_data1, rgb_data2, gt_data, gt_dense, filename) in enumerate(val_loader):
            if gt_data.ndim != 4 and gt_data[0] == False:
                continue
            img_H = gt_data.shape[2]
            img_W = gt_data.shape[3]
            gt_data = Variable(gt_data.cuda())
            input_img1, input_img2 = Variable(rgb_data1.cuda()), Variable(rgb_data2.cuda())
            ...
            input_img_flip1,input_img_flip2 = torch.flip(input_img1,[3]),torch.flip(input_img2,[3])
            with torch.no_grad():
                final_depth = test_model(input_img1, input_img2)
                final_depth_flip = test_model(input_img_flip1, input_img_flip2)
            ...
            filename = filename[0]
            img_arr = [final_depth,final_depth, final_depth, gt_data, rgb_data1, gt_dense,gt_dense,gt_dense]
