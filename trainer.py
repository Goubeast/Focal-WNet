...
def validate(args, val_loader, model, logger, dataset = 'KITTI'):
    ...
    for i, (rgb_data1, rgb_data2, gt_data, _) in enumerate(val_loader):
        if gt_data.ndim != 4 and gt_data[0] == False:
            continue
        end = time.time()
        rgb_data1 = rgb_data1.cuda()
        rgb_data2 = rgb_data2.cuda()
        gt_data = gt_data.cuda()

        # compute output
        input_img = rgb_data1, rgb_data2
        input_img_flip = torch.flip(rgb_data1,[3]), torch.flip(rgb_data2,[3])
        with torch.no_grad():
            output_depth = model(rgb_data1, rgb_data2)
            batch_time.update(time.time() - end)
            output_depth_flip = model(torch.flip(rgb_data1,[3]), torch.flip(rgb_data2,[3]))
...
def train_net(args,model, optimizer, dataset_loader,val_loader, n_epochs,logger):
    ...
    rgb_fixed, rgb512_fixed , depth_fixed, _ = next(data_iter)
    depth_fixed = depth_fixed.cuda()
    
    save_dir = './' + args.dataset + '_WNet_' + '_epoch' + str(n_epochs+5)
    ...
    for epoch in tqdm(range(n_epochs+5)):
        ...
        ####################################### one epoch training #############################################
        for i, (rgb_data, rgb512, gt_data, gt_dense) in enumerate(dataset_loader):
            ...
            depths = depths.cuda()
            inputs1, inputs2, depths = Variable(inputs1),Variable(inputs2), Variable(depths)
            ...
            grad_epoch = 15 if args.dataset == 'KITTI' else 13
            if args.use_dense_depth is True:
                if epoch < grad_epoch:
