...
class MyDataset(data.Dataset):
    def __init__(self, args, train=True, return_filename = False):
        ...
        if train is True:
            ...
            if self.dataset == 'KITTI':
                ...
                args.width = 544
        self.train = train
        self.transform = Transformer(self.dataset, args)
        self.args = args
    ...
    def __getitem__(self, index):
        ...
        if (self.dataset == 'NYU' and (self.train is False) and (self.return_filename is False)):
        ...
        transsize = transforms.Compose([transforms.Resize((224,224))])
        transsize1 = transforms.Compose([transforms.Resize((512,512))])

        rgb = transsize(rgb)
        rgb512 = transsize1(rgb)

        if self.return_filename is True:
            return rgb, rgb512, gt, gt_dense, filename
        else:
            return rgb, rgb512, gt, gt_dense
